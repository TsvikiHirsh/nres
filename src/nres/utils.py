...
def register_material(
    ...
    if isinstance(components, dict):
        ...
        msg = "Fractions must be provided either as a separate list or as dictionary values"
    ...
    for comp in components:
        ...
        # Check if component is a formula that exists in materials
            ...
            msg = f"Component {comp} not found in materials, elements, or isotopes"
            raise ValueError(msg)
    ...
    if len(resolved_components) != len(normalized_fractions):
        msg = "Number of components must match number of fractions"
        raise ValueError(msg)

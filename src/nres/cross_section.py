...
class CrossSection:
    ...
    def __init__(
        self,
        isotopes: dict[str | CrossSection, float] | None = None,
        name: str = "",
    ...
    def _get_material_data(self, material: str | dict) -> dict:
    ...
    def _set_weights(self, weights: list[float] | None = None):
        ...
        if weights is not None:
            if len(weights) != len(self.isotopes):
                msg = "Number of weights must match number of isotopes"
        ...
        for material_name, material_info in self.materials.items():
            ...
            if splitby == "isotopes":
                for element_info in material_info["elements"].values():
                    for isotope, _weight in element_info["isotopes"].items():
                        isotope_clean = isotope.replace("-", "")

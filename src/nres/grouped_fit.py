...
class GroupedFitResult:
    ...
    def __getitem__(self, index):
        ...
        if normalized_index not in self.results:
            msg = f"Index {index} not found in results. Available: {self.indices}"
            raise KeyError(msg)
    ...
    def plot(self, index, stage=None, **kwargs):
        ...
        **kwargs
        ...
        if normalized_index not in self.results:
            msg = f"Index {index} not found. Available indices: {self.indices}"
        ...
        if isinstance(fit_result, SimpleNamespace) or not hasattr(
            ...
            if model is None:
                msg = f"Cannot plot index {index}: result was loaded from file without model information."
    ...
    def plot_total_xs(self, index, stage=None, **kwargs):
        ...
        **kwargs
        ...
        if model is None or not hasattr(model, "cross_section"):
            msg = f"Cannot plot cross section for index {index}: model or cross section not available."
            raise AttributeError(msg)
        ...
        if stage is not None:
            if not hasattr(model, "fit_stages") or not model.fit_stages:
                msg = "No Rietveld stages available. Perform Rietveld fit first."
                raise ValueError(msg)

            if stage < 1 or stage > len(model.fit_stages):
                msg = f"Stage {stage} not available. Available stages: 1-{len(model.fit_stages)}"
                raise ValueError(msg)
    ...
    def _plot_multi_parameter_map(self, param_names, query=None, kind=None, **kwargs):
        ...
        **kwargs : dict
        ...
        if self.group_shape and len(self.group_shape) != 1:
            msg = "Multi-parameter plotting only works with 1D groups"
            raise ValueError(msg)
        ...
        if kind not in ["bar", "errorbar"]:
            msg = f"Multi-parameter plots only support 'bar' or 'errorbar' kind, got '{kind}'"
            raise ValueError(msg)
        ...
        **kwargs : dict, optional
        ...
        if self.group_shape and len(self.group_shape) == 2:
            ...
            if len(xs) == 0 or len(ys) == 0:
                msg = "No valid 2D indices found for plotting"
                raise ValueError(msg)
        ...
        if self.group_shape and len(self.group_shape) == 1:
            ...
            if kind == "line":
                ...
                msg = f"Unknown kind '{kind}'. Must be 'line', 'bar', or 'errorbar'."
                raise ValueError(msg)
    ...
    def save(
        self, filename: str, compact: bool = True, model_filename: str | None = None
    ):
        """

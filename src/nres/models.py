...
class TransmissionModel(lmfit.Model):
    def __init__(
        ...
        vary_weights: bool | None = None,
        vary_background: bool | None = None,
        vary_tof: bool | None = None,
        vary_response: bool | None = None,
    ...
    def transmission(
        ...
        return norm * np.exp(-xs * thickness * n) * (1 - bg) + k * bg
    ...
    def stages(self, value):
        ...
        if isinstance(value, str):
            if value != "all":
                msg = "If stages is a string, it must be 'all'"
            ...
            for stage_name, stage_def in value.items():
                if not isinstance(stage_name, str):
                    msg = f"Stage names must be strings, got {type(stage_name)}"
                    raise ValueError(msg)
                if isinstance(stage_def, str):
                    if stage_def != "all" and stage_def not in group_map:
                        msg = f"Stage definition for '{stage_name}' must be 'all' or a valid group name, got '{stage_def}'"
                        raise ValueError(msg)
                elif isinstance(stage_def, list):
                    for param in stage_def:
                        if not isinstance(param, str):
                            msg = f"Parameters in stage '{stage_name}' must be strings, got {type(param)}"
                            raise ValueError(msg)
                else:
                    msg = f"Stage definition for '{stage_name}' must be 'all', a valid group name, or a list, got {type(stage_def)}"
                    raise ValueError(msg)
            self._stages = value
        else:
            msg = f"Stages must be a string ('all') or dict, got {type(value)}"
            raise ValueError(msg)
    ...
    def fit(
        ...
        param_groups: list[list[str]] | None = None,
        **kwargs,
    ):
        """
        ...
        **kwargs,
        ...
        def resolve_single_param_or_group(item):
            ...
            if item in self.params:
                ...
                return [item]
            matching_params = [p for p in self.params if fnmatch.fnmatch(p, item)]
            if matching_params:
                if verbose:
                ...
                if isinstance(item, str):
                    if item.startswith("emin="):
                        try:
                            ...
                            warnings.warn(
                                f"Invalid emin value in group: {item}", stacklevel=2
                            )
                        ...
                        try:
                            ...
                            warnings.warn(
                                f"Invalid emax value in group: {item}", stacklevel=2
                            )
        ...
        if param_groups is None:
            ...
            msg = "param_groups must be None, a list, or a dictionary"
            raise ValueError(msg)
        ...
        if not filtered:
            msg = "No valid parameter groups found. Check your parameter names."
            raise ValueError(msg)
        ...
        for stage_idx, (stage_name, group, overrides) in enumerate(iterator):
            ...
            if isinstance(data, pd.DataFrame):
                ...
                msg = "Rietveld fitting requires energy-based input data."
                raise ValueError(msg)
            ...
            if overrides.get("pick_one", False):
                ...
                p_params = [p for p in params if re.compile(r"p\d+").match(p)]
                ...
                if len(isotope_names) <= 1:
                    ...
                    for iso_idx, isotope_name in enumerate(isotope_names):
                        ...
                        try:
                            ...
                            warnings.warn(
                                f"Fitting failed for {isotope_name}: {e}", stacklevel=2
                            )
                            isotope_results.append(
                                {
                                    "isotope": isotope_name,
            ...
            for name in cumulative_params:
                if name in params:
                    ...
                    if name in group:  # Only warn for new parameters
                        warnings.warn(
                            f"Parameter '{name}' not found in params", stacklevel=2
                        )
            ...
            try:
                ...
                if verbose:
                    warnings.warn(f"Fitting failed in {stage_name}: {e}", stacklevel=2)
                continue
        ...
        if not stage_results:
            msg = "No successful fitting stages completed"
        ...
        **kwargs,
        ...
        if data is not None and hasattr(data, "is_grouped") and data.is_grouped:
            ...
            if normalized_index not in data.groups:
                msg = f"Index {index} not found. Available indices: {data.indices}"
                raise ValueError(msg)
        ...
        if stage is not None and hasattr(self, "fit_stages") and self.fit_stages:
            ...
            if stage < 1 or stage > len(self.fit_stages):
                msg = f"Stage {stage} not available. Available stages: 1-{len(self.fit_stages)}"
                raise ValueError(msg)
            ...
            if hasattr(self, "fit_result") and self.fit_result is not None:
                ...
                msg = "Cannot plot stage results without original fit data"
                raise ValueError(msg)
    ...
    def plot_stage_progression(self, stages: list | None = None, **kwargs):
        """
        ...
        if not hasattr(self, "fit_stages") or not self.fit_stages:
            msg = "No Rietveld stages available. Run fit with method='rietveld' first."
            raise ValueError(msg)
        ...
        if hasattr(self, "fit_result") and self.fit_result is not None:
            ...
            msg = "Cannot plot stage progression without original fit data"
            raise ValueError(msg)
            ...
            if hasattr(self, "stages_summary"):
                ...
                if (stage_col, "vary") in self.stages_summary.columns:
                    varied_params = self.stages_summary.loc[
                        self.stages_summary[(stage_col, "vary")]
                    ].index.tolist()
    ...
    def get_stages_summary_table(self):
        ...
        if not hasattr(self, "stages_summary"):
            msg = "No stages summary available. Run fit with method='rietveld' first."
            raise ValueError(msg)
    ...
    def update_params(
        self, params: dict | None = None, values_only: bool = True, inplace: bool = True
    ):
        """
    ...
    def vary_all(self, vary: bool | None = None, except_for: list[str] | None = None):
        """
    ...
    def _tof_correction(
        ...
        **kwargs,
        ...
        return utils.time2energy(tof + dtof, self.cross_section.L)
    ...
    def manually_calibrate_tof(
        ...
        **kwargs,
        ...
        if inputs is None or references is None:
            msg = "Both inputs and references must be provided"
            raise ValueError(msg)
        ...
        if len(inputs) != len(references):
            msg = "Input values and reference values must have the same length"
            raise ValueError(msg)
        ...
        if input_type == "energy":
            ...
            msg = "Invalid input_type. Must be 'tof', 'energy', or 'slice'"
            raise ValueError(msg)
        ...
        if reference_type == "energy":
            ...
            msg = "Invalid reference_type. Must be 'tof', 'energy', or 'slice'"
            raise ValueError(msg)
        ...
        self.params.set(t0={"value": result.params["t0"].value, "vary": False})
        self.params.set(L0={"value": result.params["L0"].value, "vary": False})
